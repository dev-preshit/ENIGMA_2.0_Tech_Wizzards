    ...
    <div className="max-w-5xl mx-auto px-4">
      ...
      <div className="grid md:grid-cols-3 gap-6 relative">
        ...
        {[
          ...
          { step: '02', icon: Brain,       title: 'AI Analysis',      desc: 'Our MobileNetV2 model analyzes the image across 7 skin lesion categories', color: '#8b5cf6' },
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        ...
        <p className="text-gray-500 dark:text-[#6b8fc2] mt-2 font-medium">Upload a clear photo of the lesion for instant AI analysis across 7 lesion categories</p>
      </motion.div>

      <UploadCard onImageSelect={(file) => { setSelectedImage(file); setResult(null); setError(null) }} isLoading={isAnalyzing} />
        ...
        {result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8 space-y-6">
            <div className="flex justify-end">
              ...
              >
                {downloading ? <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Generating...</> : <><Download className="w-4 h-4" />Download Report</>}
              </motion.button>
            </div>
      ...
      {/* Tips */}
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mt-12 p-5 rounded-2xl border bg-amber-50 border-amber-200 dark:bg-amber-900/15 dark:border-amber-800/40">
